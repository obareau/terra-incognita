...
| 19 | **Zoom continu inter-échelles** : la molette franchit désormais les 5 échelles sans passer par un bouton — dézoomer au minimum de l'échelle courante remonte d'un cran (ville→région→planète→système), zoomer au maximum sur un POI survolé descend d'un cran (système→planète→région→ville→intérieur) ; vue système (sans camera zoom) recalcule la planète survolée directement depuis les coordonnées de l'évènement molette (robuste même sans mousemove préalable) | testé en navigateur : cycle complet système↔planète↔région↔ville par simple molette, aucune régression sur le pan/zoom existant à l'intérieur d'une échelle | ✅ 2026-07-09 |

**MVP = phases 0–2 — atteint. v0.3.0 : styles de faction, monuments, web + radio. v0.4 (en cours) : planète + climat + système + météo + zoom continu.**
